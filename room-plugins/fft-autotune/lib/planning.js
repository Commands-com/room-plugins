  ...
  return getLaneParticipantViews(ctx, state)
    ...
    .map(({ participant, lane, laneIndex }) => ({
      ...
      message: [
        ...
        'Write the compiled binary, validation JSON, benchmark JSON, and every reported artifact path inside the FFT output directory above. Do NOT write validation/benchmark evidence to /tmp or any path outside the workspace/output directory.',
        'Every reported binaryPath, validationPath, samplePath, and artifactPaths entry must resolve inside the workspace/output directory or the room will reject the result.',
        '',
