...
export function buildPlanningPrompt(objective, workers, domainContext = '', options = {}) {
  ...
  return [
    ...
    '- When practical, include the direct verification step in the task description (target test, build, command, or manual check)',
...
export function buildTaskAssignmentPrompt(task, taskBoard, upstreamContext = '', options = {}) {
  ...
  const writeGuidance = task?.requiresWrite === false
    ...
    : [
        ...
        'Verify the task with the narrowest direct check available and mention the result in your summary.',
      ];
  return [
    `## Task: ${task.title}`,
    '',
...
export function buildResultProcessingPrompt(completedTasks, taskBoard, objective, options = {}) {
  ...
  return [
    'You are the General reviewing completed work and planning next steps.',
    '',
    '## Objective',
    objective,
    '',
